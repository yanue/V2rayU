...
actor LegacyMigrationHandler {
    ...
    ///
    /// 解析 outbound 配置中的第一个代理服务器配置
    ///
    /// - Parameter json: V2ray JSON 配置字符串
    /// - Returns: 解析后的 ProfileEntity，失败返回 nil
    private func parseFromJson(json: String) -> ProfileEntity? {
        guard let data = json.data(using: .utf8) else {
            logger.debug("LegacyMigration: JSON parse failed - invalid string encoding")
            return nil
        }
        ...
        do {
            ...
            guard let jsonObj = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                  let outbounds = jsonObj["outbounds"] as? [[String: Any]],
                  let firstOutbound = outbounds.first,
                  let protocolStr = firstOutbound["protocol"] as? String else {
                logger.debug("LegacyMigration: JSON parse failed - missing required fields")
                return nil
            }

            logger.debug("LegacyMigration: Parsing JSON protocol '\(protocolStr)'")
            return parseOutboundToProfile(protocolStr: protocolStr, outbound: firstOutbound, jsonObj: jsonObj)

        } catch {
            logger.error("LegacyMigration: JSON parse error: \(error)")
            return nil
        }
    }
    ...
    /// - Parameters:
    ///   - protocolStr: 协议类型字符串
    ///   - outbound: outbound 配置字典
    ///   - jsonObj: 完整的 JSON 配置（用于查找 DNS 等额外信息）
    /// - Returns: 解析后的 ProfileEntity
    private func parseOutboundToProfile(protocolStr: String, outbound: [String: Any], jsonObj: [String: Any]) -> ProfileEntity? {
        var profile = ProfileEntity()
        ...
        default:
            logger.warning("LegacyMigration: Unsupported protocol '\(protocolStr)'")
            return nil
        }
    ...
    ///
    /// - Parameters:
    ///   - streamSettings: streamSettings 配置字典
    ///   - profile: 要填充的 ProfileEntity（inout）
